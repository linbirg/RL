...
class ACNet(object):
    ...
    def _build_net(self, scope):
        ...
        with tf.variable_scope('actor'):
            l_a = tf.layers.dense(
                self.s, 200, tf.nn.relu6, kernel_initializer=w_init, name='la')
            a_prob = tf.layers.dense(
                l_a, N_A, tf.nn.softmax, kernel_initializer=w_init, name='ap')
        with tf.variable_scope('critic'):
            l_c = tf.layers.dense(
                self.s, 100, tf.nn.relu6, kernel_initializer=w_init, name='lc')
            v = tf.layers.dense(
...
if __name__ == "__main__":
    ...
    with tf.device("/cpu:0"):
        ...
        GLOBAL_AC = ACNet(GLOBAL_NET_SCOPE, N_S, N_A)  # we only need its params
        workers = []
        # Create worker
