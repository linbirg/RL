...
class Maze(object):
    ...
    def save(self, filename="maze.json"):
        import json
        maze_dict = {
            "bounds": (self.max_x, self.max_y),
            "start": (self.x, self.y),
            "door": self.door,
            "blocks": self.blocks
        }
        with open(filename, 'w') as outfile:
            json.dump(maze_dict, outfile, ensure_ascii=False)
            outfile.write('\n')
    ...
    @classmethod
    def reload(cls, filename="maze.json"):
        import os
        import json
        maze_tmp = None
        if os.path.exists(filename):
            with open(filename) as f:
                ...
                maze_tmp = Maze.build(
                    bounds=maze_dict["bounds"],
                    door=maze_dict["door"],
                    blocks=maze_dict["blocks"])
                maze_tmp.set_start(start=maze_dict["start"])
        return maze_tmp
